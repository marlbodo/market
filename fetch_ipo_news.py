...
# We also use this same list to filter by TITLE only (see fetch_naver_ipo_news_for_keyword),
# so search and filtering stay consistent — no separate "content keyword" search.
# Kept intentionally narrow (공모주, IPO only) — broader words like 청약/상장/공모가
# tend to pull in unrelated finance news.
TITLE_FILTER_WORDS = ["공모주", "IPO"]

# Titles containing any of these words are dropped even if they'd otherwise
# pass the TITLE_FILTER_WORDS check. Empty for now — add words here if noisy
# patterns show up once this is running (same approach as fetch_news.py).
TITLE_EXCLUDE_WORDS = []

# Some filter words can be substrings of unrelated words, in which case a
# plain "word in title" check would produce false positives. None identified
# yet for 공모주/IPO, but the mechanism is here (see fetch_news.py's "채권"
# vs "채권자" handling) in case one turns up.
...
def is_near_duplicate_title(title, existing_titles, threshold=0.6):
    ...
    # reworded headlines. An exact-string dedupe misses these, so we also
    # compare similarity ratios and drop close matches.
...
def fetch_naver_ipo_news(max_total=30):
    ...
    for keyword in TITLE_FILTER_WORDS:
        for item in fetch_naver_ipo_news_for_keyword(keyword):
...
def main():
    ...
    news_items = fetch_naver_ipo_news()
    if not news_items:
        print("가져온 뉴스가 없습니다.")
        return
    ...
    try:
        supabase.table("ipo_news").delete().gt("id", 0).execute()
        print("기존 ipo_news 테이블 데이터를 모두 비웠습니다.")
        ...
        supabase.table("ipo_news").insert(rows_to_insert).execute()
        print(f"성공적으로 {len(rows_to_insert)}개의 최신 공모주 뉴스를 Supabase에 저장했습니다.")
    except Exception as e:
